...
THREEx.DayNight.StarField	= function(){
	// create the mesh
	var texture	= new TextureLoader().load(THREEx.DayNight.baseURL+'images/galaxy_starfield.png')
...
THREEx.DayNight.SunLight	= function(){
	...
	var distance = 400;
	var square = 30;
	light.shadow.camera.near = distance-square;
	light.shadow.camera.far = distance+square;
	light.shadow.camera.left = -square;
	light.shadow.camera.right = square;
	light.shadow.camera.top = square;
	light.shadow.camera.bottom = -square;
	...
	this.update	= function(sunAngle){
		light.position.x = 0;
		light.position.y = Math.sin(sunAngle) * distance;
		light.position.z = Math.cos(sunAngle) * distance;
		//light.shadow.camera.updateProjectionMatrix();
...
THREEx.DayNight.SunSphere	= function(){
	var geometry	= new SphereGeometry( 20, 30, 30 )
	var material	= new MeshBasicMaterial({
		color		: 0xff0000
	})
	var mesh	= new Mesh(geometry, material)
...
THREEx.DayNight.Skydom		= function(){
	var geometry	= new SphereGeometry( 700, 32, 15 );
...
THREEx.DayNight.Skydom.Shader	= {
	uniforms	: {
		topColor	: { type: "c", value: new Color().setHSL( 0.6, 1, 0.75 ) },
